"""Pool a variable number of per-cell embeddings into one vector per patch.

Three pooling choices, all confidence-weighted and all reporting an explicit
empty-patch mask (`reliability`):

* `pool_cells_mean` — the first moment. Pooling features before a shared
  linear head is exactly equivalent to pooling that head's per-cell logits,
  while training each patch only once.
* `pool_cells_rff` — a random-Fourier approximation of the Gaussian kernel
  mean embedding, i.e. a scalable stand-in for the patch's cell-distribution
  KDE. Unlike the mean it separates bags with the same first moment but
  different modes.
* `pool_cells_moments` — mean concatenated with spread, a cheap middle ground.
"""
