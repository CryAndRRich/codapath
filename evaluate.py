...
def evaluate_model(probe: LinearProbe,
...
def visualize_tsne(features: np.ndarray,
    ...
    sns.scatterplot(
        x=X_2d[:, 0], y=X_2d[:, 1],
        hue=label_names, hue_order=hue_order,
...
def plot_acc_diff(budgets_dict: Dict[str, List[int]],
    ...
    styles = {
        ...
        "CODAPath":  {"color": "darkblue", "marker": "*", "zorder": 10,
    ...
    for col in range(3):
        ...
        ax.set_title(datasets_title[col], fontsize=14, pad=10)
        ax.grid(True, linestyle="--", alpha=0.5)

    fig.legend(
        loc="upper center", bbox_to_anchor=(0.5, 1.15),
        ncol=8, fontsize=12, frameon=True, shadow=True
    )
    plt.tight_layout()
