"""Candidate-weighted nucleus coverage with CellViT coverage features.

Companion to `sampling/nucleus_al.py`, which uses CellViT for uncertainty on a
separate controlled axis. Here uncertainty is fixed and simple (plain margin
on a DINOv2+Linear probe); the experimental axis is which feature space the
coverage kernel operates in: `dino` | `cellvit` | `concat`. The Kaggle entry
point is `scripts/run_nucleus_coverage.ipynb`.
...
2. `Score(i) = minmax(Uncertainty)(i) * minmax(Coverage)(i)` departs from
   ...
   itself). The greedy/radius mechanics are UHerding-inspired, while the
   Gaussian helper inherits SCALPEL's `exp(-(1-cos)/sigma^2)` convention
   (equivalent to an RBF with a sqrt(2)-rescaled Euclidean bandwidth). The
   final combination is a self-designed hybrid. Since
   the two min-max normalizers depend on the remaining candidate pool, this is
   not one fixed submodular set objective and must not be assigned UHerding's
   `(1 - 1/e)` greedy guarantee.
